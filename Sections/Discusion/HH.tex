...
Mathematically, the flow of current is represented as the change in voltage over time scaled by capacitance:
    ...
    \intertext{and the current through a given \gls{gls:ionChan} is the product of that channel's \gls{gls:conductance} and the \gls{gls:rPote} for the specific ion}
...
\end{alignat}
Where \(C_m\) the membrane capacitance per unit area; and \({g}_\rmm{K}\), \(V_\rmm{K}\), along with  \({g}_\rmm{Na}\), \(V_\rmm{Na}\) make up the \gls{gls:conductance} and \gls{gls:rPote} of \gls{K} and \gls{Na} respectively.
...
\begin{table}[htb]
    ...
    %t &time & ms\\
    \unit{\maxcon\potassium} & maximal conductance for \gls{K} & \qty{36}{m.mho/cm2} \\
    \unit{\maxcon\sodium} & maximal conductance for \gls{Na} &\qty{120}{m.mho/cm2} \\
