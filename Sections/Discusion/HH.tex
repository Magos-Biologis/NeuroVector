...
Mathematically, the flow of current is represented as the change in voltage over time scaled by capacitance:
    \begin{align}
        I_c &= \capa\membrane \ode{\unit{\V\membrane}} \\
    \intertext{and recall that the current through a given \gls{gls:ionChan}, \cref{eq:ionCurrent}, is the product of that channel's \gls{gls:contan} and the \gls{gls:rPote} for the specific ion}
...
\begin{equation}
     \unit{\curr}  =  I_c + \sum_{i = 1}^{p} I_i =  I_c + \sum_{i = 1}^{p} g_\bfm{ion} \cdot \br{\unit{\V\membrane}-\unit{\V\ion}} 
\end{equation} 
with \(I\) representing the total membrane current per unit area; 
\begin{alignat}{5}
    &\quad \unit{\curr} &&= \quad I_c \quad&&+ \quad I_\rmm{K} \quad&&+ \quad I_\rmm{Na} \quad&&+ \quad I_\rmm{L} \\ 
    \implies &\quad \unit{\curr} &&= C\mm \ode{\unit{\V\membrane}} &&+ {g}_\rmm{K}\br{\unit{\V\membrane} - V_\rmm{K}} &&+ {g}_\rmm{Na} \br{\unit{\V\membrane} - V_\rmm{Na}}  &&+ {g}_\rmm{L} \br{\unit{\V\membrane} - V_\rmm{L}} 
\end{alignat}
Where \(C\mm\) the membrane capacitance per unit area; and \({g}_\rmm{K}\), \(V_\rmm{K}\), along with  \({g}_\rmm{Na}\), \(V_\rmm{Na}\) make up the \gls{gls:cond} and \gls{gls:rPote} of \gls{K} and \gls{Na} respectively.
...
\begin{sysEquation}[HodHuxSys]
    C\mm \ode{\unit{\V\membrane}} &= I\mm - \br{\bar{g}_\rmm{K} n^4 \br{\unit{\V\membrane} - V_\rmm{K}} + \bar{g}_\rmm{Na} m^3 h \br{\unit{\V\membrane} - V_\rmm{Na}}  + \bar{g}_\rmm{L} \br{\unit{\V\membrane} - V_\rmm{L}}} \\
    \ode{n} &= \alpha\nn \br{\unit{\V\membrane}} \br{1-n} - \beta\nn \br{\unit{\V\membrane}} n \\
    \ode{m} &= \alpha\mm \br{\unit{\V\membrane}} \br{1-m} - \beta\mm \br{\unit{\V\membrane}} m \\
    \ode{h} &= \alpha\hh \br{\unit{\V\membrane}} \br{1-h} - \beta\hh \br{\unit{\V\membrane}} h 
\end{sysEquation}
...
\begin{table}[htb]
    ...
    \unit{\curr} & Membrane Current Density 0A/cm2 during space clamp & \\
