...
%\glspl{gls:membrane} are maintained, to varying degrees, in all cellular organisms \cite{ Hammond2015ch4}. 
...
%They observed a key phenomenon, that the AP is underlied by two separate voltage-dependent currents: an early transient \gls{Na} \textit{inward} current, which depolarizes the cell membrane, and a delayed \gls{K} \textit{outward} current, largely responsible for re-polarization.}. \cite{kandel2000principles}. 
...
%The expected \gls{gls:mPote} in human cells is \glslink{gls:volt}{\qty{-70}{\milli\volt}}
%\glslocalreset{ap}, is the measure of the \gls{gls:Pote} that causes action ~\cite{Hammond2015ch4}.
